...
** The =early-init.el= sets the PATH and exec-path

Explicitly sets the PATH environment variable inside Emacs when started from the GUI.
Sets Emacs' exec-path to match this value.
These values are copied from ~$(brew --cellar emacs-plus@31)/31.0.50/Emacs.app/Contents/Info.plist~
...
*** The =detvdl-emacs-essentials.el= block for ~direnv~
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-essentials.el"
(use-package direnv
  :ensure t
  :demand t
