...
TODO: add smartparens or similar, aggressive-indent, and programming-language specific setups (LSP)
...
minibuffer, hence the function ~emacs-no-minibuffer-scroll-bar~.
...
- [[#h:36af519a-67e1-42b6-a38a-2052638108f9][The =detvdl-emacs-modus-themes.el= module]]
...
All user options must be set in a =emacs-pre-custom.el= file in
...
I normally use ~vertico~ for minibuffer UI elements.
Here we specify which module to load at startup. Remember to read how
these options come into effect ([[#h:5a41861f-4c38-45ac-8da2-51d77c0b4a73][The init.el user options]]).
...
- [[#h:22e97b4c-d88d-4deb-9ab3-f80631f9ff1d][The =devtdl-emacs-completion.el= settings for ~consult~]]
- [[#h:61863da4-8739-42ae-a30f-6e9d686e1995][The =detvdl-emacs-completion.el= section about ~embark~]]
...
Also check [[#h:ddb1070d-2f91-4224-ad43-ef03f038f787][the =detvdl-emacs-which-key.el= module]].
...
Also check [[#h:4efde779-3ef0-40b7-82e4-70f71b321428][the =detvdl-emacs-icons.el= module]].
...
- The =detvdl-emacs-modules= directory :: This is where I store all the
  ...
  with =detvdl-emacs-=, followed by a word that broadly describes their
  scope of application, such as ~detvdl-emacs-font~, ~detvdl-emacs-window~...
  ...
  the file name minus its file type extension: ~detvdl-emacs-theme~ is
  the feature provided by =detvdl-emacs-theme.el=.
...
==emacs-init.org= file or its web page version. Still, this arrangement
...
#+end_src
** MAYBE The =init.el= settings for package upgrades (~package-upgrade-guard~)

Package Upgrade Guard is an Emacs security tool that shows diffs for
all package upgrades and installations, helping users review changes
before proceeding. It supports both ELPA/MELPA packages and VC
(version control) packages.

Source: https://github.com/kn66/package-upgrade-guard.el
...
** The =init.el= sets the PATH and exec-path

Explicitly sets the PATH environment variable inside Emacs when
started from the GUI.
This is needed due to: https://github.com/d12frosted/homebrew-emacs-plus/issues/720
...
[ Note that Emacs considers the built-in themes "safe". ]

#+begin_src emacs-lisp :tangle "init.el"
(setq custom-safe-themes t)
#+end_src

** The =init.el= macro to bind keys (~detvdl-emacs-keybind~)
:PROPERTIES:
...
** The =init.el= macro to define abbreviations (~detvdl-emacs-abbrev~)
:PROPERTIES:
...
This is the same idea as ~detvdl-emacs-keybind~, adjusted to work with
the ~define-abbrev~ function ([[#h:d3684ec0-a972-4033-a4d6-41ee16e891d9][The init.el macro to bind keys (~detvdl-emacs-keybind~)]]).
...
n** The =init.el= addition of highlighting for my macros
:PROPERTIES:
...
those options must have the =detvdl-emacs-pre-custom.el= file in place,
...
loaded at the end of my =init.el= ([[#h:e6c4acf5-5b51-4b38-a86a-bf3f698ac872][The init.el final part to load the individual

** The =detvdl-emacs-theme.el= module
:PROPERTIES:
...
*** The =detvdl-emacs-theme.el= section to load a theme (~detvdl-emacs-load-theme-family~)
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-theme.el" :mkdirp yes
...
**** The =detvdl-emacs-modus-themes.el= module
:PROPERTIES:
:CUSTOM_ID: h:36af519a-67e1-42b6-a38a-2052638108f9
:END:

This is one of the components of the =detvdl-emacs-theme.el= module
([[#h:3f1a33e4-762e-4cf3-a0ae-4c2954d9cbb8][The =detvdl-emacs-theme.el= section to load a theme (~detvdl-emacs-load-theme-family~)]]).
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-modus-themes.el"
;;; The Modus themes
...
*** The =detvdl-emacs-theme.el= section for ~lin~
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-theme.el"
;;;; Lin
...
*** The =detvdl-emacs-theme.el= section for ~spacious-padding~
:PROPERTIES:
...
([[#h:41ee38d0-9099-4444-85e4-cc8dce00fb9a][The =detvdl-emacs-modeline.el= module]]).
...
*** The =detvdl-emacs-theme.el= section for ~rainbow-mode~
:PROPERTIES:
...
*** The =detvdl-emacs-theme.el= section about ~fontaine~
:PROPERTIES:
...
([[#h:60d6aae2-6e4b-402c-b6a8-411fc49a6857][The =detvdl-emacs-theme.el= section about ~variable-pitch-mode~ and font resizing]]).

Also check my ~show-font~ package ([[#h:60a005be-77bd-49f1-a865-78d7cf75bd2a][The =detvdl-emacs-theme.el= section about ~show-font~]]).
...
*** The =detvdl-emacs-theme.el= section about ~show-font~
:PROPERTIES:
...
To actually set fonts, I use my ~fontaine~ package ([[#h:cb41fef0-41a5-4a85-9552-496d96290258][The =detvdl-emacs-theme.el= section about ~fontaine~]]).
...
*** The =detvdl-emacs-theme.el= section about ~variable-pitch-mode~ and font resizing
:PROPERTIES:
...
families in my ~fontaine~ setup ([[#h:cb41fef0-41a5-4a85-9552-496d96290258][The =detvdl-emacs-theme.el= section about ~fontaine~]]).
...
*** The =detvdl-emacs-theme.el= call to ~provide~
:PROPERTIES:
...
** The =detvdl-emacs-essentials.el= module
:PROPERTIES:
...
*** The =detvdl-emacs-essentials.el= block with basic configurations
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-essentials.el" :mkdirp yes
...
*** The =detvdl-emacs-essentials.el= block with sane defaults (crux)
Crux is a set of "sane" default behaviours for common functionality,
such as killing lines, moving to the beginning of a line, etc...

It also provides advices that act in a more "do-what-I-mean (dwim)"
fashion when wrapped around regular functions

Source: https://github.com/bbatsov/crux

#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-essentials.el"
(use-package crux
  :ensure t
...
*** The =detvdl-emacs-essentials.el= block for ~mise~ (direnv/asdf)

Mise is a tool version manager for common binaries, but also other
package-managed tools (via e.g. ~cargo~ or ~go~).
...
go = "1.21"
node = "24.5.0"
"asdf:clojure" = "1.10.3.1040"
"aqua:terraform" = "1.12.2"
java = "17.0.2"
"asdf:kotlin" = "2.2.0"
uv = "latest"
ruff = "latest"
...
Source: https://github.com/eki3z/mise.el
TODO: figure out why this resets the opam path
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-essentials.el"
;; (use-package mise
;;   :ensure t
...
*** The =detvdl-emacs-essentials.el= section about =prot-common.el= (custom basic functions)
:PROPERTIES:
...
*** The =detvdl-emacs-essentials.el= section about =prot-simple.el= (custom basic commands)
:PROPERTIES:
...
*** The =detvdl-emacs-essentials.el= section about =prot-scratch.el= (scratch buffer per major mode)
:PROPERTIES:
...
*** The =detvdl-emacs-essentials.el= section about =prot-pair.el= (insert character pairs)
:PROPERTIES:
...
*** COMMENT The =detvdl-emacs-essentials.el= section for comments
:PROPERTIES:
...
(use-package prot-comment
  ...
  :bind
  (("C-x C-;" . prot-comment)
   ("C-c C-;" . prot-comment-timestamp-keyword)))
#+end_src

*** The =detvdl-emacs-essentials.el= section about =prot-prefix.el= (prefix nested keymaps)
:PROPERTIES:
...
configurable amount of time ([[#h:ddb1070d-2f91-4224-ad43-ef03f038f787][The =detvdl-emacs-which-key.el= module]]).
...
*** The =detvdl-emacs-essentials.el= configuration to track recently visited files
:PROPERTIES:
...
completion ([[#h:15edf2c3-4419-4101-928a-6e224958a741][The =detvdl-emacs-completion.el= module]]).
...
bookmarks ([[#h:22e97b4c-d88d-4deb-9ab3-f80631f9ff1d][The =detvdl-emacs-completion.el= settings for ~consult~]]).
...
*** The =detvdl-emacs-essentials.el= mouse configurations and scroll behaviour
:PROPERTIES:
...
*** The =detvdl-emacs-essentials.el= settings for ~repeat-mode~
:PROPERTIES:
...
*** The =detvdl-emacs-essentials.el= settings for registers
:PROPERTIES:
...
quickly ([[#h:581aa0ff-b136-4099-a321-3b86edbfbccb][The =detvdl-emacs-essentials.el= settings for bookmarks]]). A
...
with ~savehist-mode~ ([[#h:25765797-27a5-431e-8aa4-cc890a6a913a][The =detvdl-emacs-completion.el= settings for saving the history (~savehist-mode~)]]).
...
*** The =detvdl-emacs-essentials.el= settings for auto revert
:PROPERTIES:
...
*** The =detvdl-emacs-essentials.el= section for ~delete-selection-mode~
:PROPERTIES:
...
*** The =detvdl-emacs-essentials.el= settings for tooltips
:PROPERTIES:
...
*** The =detvdl-emacs-essentials.el= settings for manpages
:PROPERTIES:
:CUSTOM_ID: h:5eb16d63-ce21-4e76-8789-eef6ddbd7c00
:END:

Most buffers conform with rules we define in the ~display-buffer-alist~
([[#h:b5fa481d-8549-4424-869e-91091cdf730b][The =detvdl-emacs-window.el= module]]). However, =M-x man= does not do
...
*** The =detvdl-emacs-essentials.el= section about ~substitute~
:PROPERTIES:
...
*** The =detvdl-emacs-essentials.el= section about ~goto-chg~ (go to change)
:PROPERTIES:
...
*** The =detvdl-emacs-essentials.el= section about ~expreg~ (tree-sitter mark syntactically)
:PROPERTIES:
...
([[#h:f15bc843-2dd1-4792-95ec-9b7b6e561804][The =detvdl-emacs-essentials.el= section about =prot-simple.el= (custom basic commands)]]).
...
*** The =detvdl-emacs-essentials.el= section about ~multi-editing~ (multiple-cursors, iedit)
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-essentials.el"
...
*** COMMENT The =detvdl-emacs-essentials.el= section about ~vundo~ (visualise undo steps)
:PROPERTIES:
...
visualisation is a nice extra ([[#h:d3684ec0-a972-4033-a4d6-41ee16e891d9][The =init.el= macro to bind keys (~detvdl-emacs-keybind~)]]).
...
*** The =detvdl-emacs-essentials.el= section about generic shells and REPLs (~comint~)
:PROPERTIES:
...
elsewhere ([[#h:7c25e281-d92e-4605-b5d6-18fe1f280a73][The =detvdl-emacs-essentials.el= section about ~shell~]]), as
...
([[#h:61c101b0-fd5e-41ba-9031-9236851af696][The =detvdl-emacs-essentials.el= section about the compilation interface (~compile~)]]).
...
*** The =detvdl-emacs-essentials.el= section about the compilation interface (~compile~)
:PROPERTIES:
:CUSTOM_ID: h:61c101b0-fd5e-41ba-9031-9236851af696
:END:

Similar to the ~comint~ library ([[#h:3ce29c2d-27ce-4f04-86ff-6acaf9426604][The =detvdl-emacs-essentials.el= section about generic shells and REPLs (~comint~)]]),
...
~project-compile~ ([[#h:7dcbcadf-8af6-487d-b864-e4ce56d69530][The =detvdl-emacs-git.el= section about =project.el=]]).
...
*** The =detvdl-emacs-essentials.el= section about ~shell~
:PROPERTIES:
...
- [[#h:3ce29c2d-27ce-4f04-86ff-6acaf9426604][The =detvdl-emacs-essentials.el= section about generic shells and REPLs (~comint~)]].
- [[#h:61c101b0-fd5e-41ba-9031-9236851af696][The =detvdl-emacs-essentials.el= section about the compilation interface (~compile~)]].
...
terminal ([[#h:14d40e5a-327e-453e-861c-3ece24351d62][The =detvdl-emacs-essentials.el= settings for ~proced~]]).
...
*** The =detvdl-emacs-essentials.el= call to ~provide~
:PROPERTIES:
...
** The =detvdl-emacs-modeline.el= module
:PROPERTIES:
...
([[#h:d4e30bcb-2f90-445f-a5ca-856e2576483a][The =detvdl-emacs-essentials.el= configurations for the date and time (~display-time-mode~)]])
or the ~notmuch-indicator~ ([[#h:0f12f144-ca9f-4d07-8563-c33b3521bfb5][The =detvdl-emacs-notmuch.el= section about the ~notmuch-indicator~]]).
...
~spacious-padding~ package ([[#h:6c118185-fcb1-4c9a-93af-71814cb84279][The =detvdl-emacs-theme.el= section for ~spacious-padding~]]).
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-modeline.el" :mkdirp yes
;;; Mode line
...
*** The =detvdl-emacs-modeline.el= section about ~keycast~
:PROPERTIES:
...
with my customised mode line ([[#h:41ee38d0-9099-4444-85e4-cc8dce00fb9a][The =detvdl-emacs-modeline.el= module]]).

#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-modeline.el"
;;; Keycast mode
(use-package keycast
  :ensure t
  :after prot-modeline
...
** The =detvdl-emacs-completion.el= module
:PROPERTIES:
...
*** The =detvdl-emacs-completion.el= settings for completion styles
:PROPERTIES:
...
- orderless :: This is the only completion style I use which is not
  ...
  ([[#h:7cc77fd0-8f98-4fc0-80be-48a758fcb6e2][The =detvdl-emacs-completion.el= for the ~orderless~ completion style]]).
...
([[#h:0d49218d-4199-4847-9765-60a3f94e515d][The =detvdl-emacs-completion.el= settings for completion category overrides]]).

#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-completion.el" :mkdirp yes
...
*** The =detvdl-emacs-completion.el= settings for completion category overrides
:PROPERTIES:
:CUSTOM_ID: h:0d49218d-4199-4847-9765-60a3f94e515d
:END:

This builds on the code we have for the ~completion-styles~
([[#h:14b09958-279e-4069-81e3-5a16c9b69892][The =detvdl-emacs-completion.el= settings for completion styles]]).
...
*** The =detvdl-emacs-completion.el= for the ~orderless~ completion style
:PROPERTIES:
:CUSTOM_ID: h:7cc77fd0-8f98-4fc0-80be-48a758fcb6e2
:END:

The ~orderless~ package by Omar Antolín Camarena provides one of the
completion styles that I use ([[#h:14b09958-279e-4069-81e3-5a16c9b69892][The =detvdl-emacs-completion.el= settings for completion styles]]).
...
annotations displayed by ~marginalia~ ([[#h:bd3f7a1d-a53d-4d3e-860e-25c5b35d8e7e][The =detvdl-emacs-completion.el= section to configure completion annotations (~marginalia~)]]).
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-completion.el"
...
*** The =detvdl-emacs-completion.el= settings to ignore letter casing
:PROPERTIES:
...
*** The =detvdl-emacs-completion.el= settings for recursive minibuffers
:PROPERTIES:
...
*** The =detvdl-emacs-completion.el= settings for default values
:PROPERTIES:
...
*** The =detvdl-emacs-completion.el= settings for common interactions
:PROPERTIES:
...
*** The =detvdl-emacs-completion.el= generic minibuffer UI settings
:PROPERTIES:
:CUSTOM_ID: h:de61a607-0bdf-462b-94cd-c0898319590e
:END:

These are some settings for the default completion user interface.
These do not come into effect unless ~detvdl-emacs-completion-ui~ is set
...
- [[#h:cff33514-d3ac-4c16-a889-ea39d7346dc5][The =detvdl-emacs-completion.el= submodule for ~vertico~ (=detvdl-emacs-vertico.el=)]]
- [[#h:e9fb09f0-f2e6-4943-83bf-e5fadcfa530a][The =detvdl-emacs-completion.el= submodule for ~mct~ (=detvdl-emacs-mct.el=)]]
...
*** The =detvdl-emacs-completion.el= settings for saving the history (~savehist-mode~)
:PROPERTIES:
...
- [[#h:804b858f-7913-47ef-aaf4-8eef5b59ecb4][The =detvdl-emacs-completion.el= for in-buffer completion popup and preview (~corfu~)]]
- [[#h:5685df62-4484-42ad-a062-d55ab19022e3][The =detvdl-emacs-essentials.el= settings for registers]]
...
*** The =detvdl-emacs-completion.el= settings for dynamic text expansion (~dabbrev~)
:PROPERTIES:
...
static, user-defined abbreviations ([[#h:fd84b79a-351e-40f0-b383-bf520d77834b][The =detvdl-emacs-completion.el= settings for static text expansion (~abbrev~)]]).
...
*** The =detvdl-emacs-completion.el= settings for static text expansion (~abbrev~)
:PROPERTIES:
...
to what we have specified ([[#h:567bb00f-1d82-4746-93e5-e0f60721728a][The =detvdl-emacs-completion.el= settings for dynamic text expansion (~dabbrev~)]]).
...
As you will notice below, I use the ~detvdl-emacs-abbrev~ macro to make
it easier to define these ([[#h:e7a12825-7848-42bd-b99b-b87903012814][The =init.el= macro to define abbreviations (~detvdl-emacs-abbrev~)]]).
...
*** The =detvdl-emacs-completion.el= for in-buffer completion popup (~corfu~)
:PROPERTIES:
...
Also see [[#h:567bb00f-1d82-4746-93e5-e0f60721728a][the =detvdl-emacs-completion.el= settings for dynamic text expansion (~dabbrev~)]].
...
*** The =detvdl-emacs-completion.el= settings for ~consult~
:PROPERTIES:
...
- A case where filtering is in use is the ~consult-buffer~ command,
  ...
  bookmarks ([[#h:581aa0ff-b136-4099-a321-3b86edbfbccb][The =detvdl-emacs-essentials.el= settings for bookmarks]]),
...
- Asynchronous input pertains to the intersection between Emacs and
  ...
  (e.g. with help from ~orderless~ ([[#h:7cc77fd0-8f98-4fc0-80be-48a758fcb6e2][The =detvdl-emacs-completion.el= for the ~orderless~ completion style]]).
...
Also check: [[#h:e0f9c30e-3a98-4479-b709-7008277749e4][The =detvdl-emacs-search.el= module]].
...
*** The =detvdl-emacs-completion.el= section about ~embark~
:PROPERTIES:
...
do with all well-behaved buffers ([[#h:50f8b1e4-b14e-453f-a37e-1c0e495ab80f][The =detvdl-emacs-window.el= rules for displaying buffers (~display-buffer-alist~)]]).
...
Another downside, which is also true for ~which-key~ ([[#h:ddb1070d-2f91-4224-ad43-ef03f038f787][The =detvdl-emacs-which-key.el= module]]),
...
*** The =detvdl-emacs-completion.el= section to configure completion annotations (~marginalia~)
:PROPERTIES:
...
accordingly ([[#h:14b09958-279e-4069-81e3-5a16c9b69892][The =detvdl-emacs-completion.el= settings for completion styles]]).
...
*** The =detvdl-emacs-completion.el= setting to load a minibuffer UI submodule
:PROPERTIES:
...
**** The =detvdl-emacs-completion.el= submodule for ~vertico~ (=detvdl-emacs-vertico.el=)
:PROPERTIES:
:CUSTOM_ID: h:cff33514-d3ac-4c16-a889-ea39d7346dc5
:END:

[ This is subject to a user option: [[#h:db620e44-fc8f-45b8-a7fd-ae1d0b7ce86c][The =detvdl-emacs-completion.el= setting to load a minibuffer UI submodule]]. ]
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-vertico.el" :mkdirp yes
...
*** The =detvdl-emacs-completion.el= section to configure AI-assisted coding

Claude Code IDE for Emacs provides native integration with Claude Code
CLI through the Model Context Protocol (MCP). Unlike simple terminal
wrappers, this package creates a bidirectional bridge between Claude
and Emacs, enabling Claude to understand and leverage Emacs’ powerful
features—from LSP and project management to custom Elisp functions.
This transforms Claude into a true Emacs-aware AI assistant that works
within your existing workflow and can interact with your entire Emacs
ecosystem. 

Source: https://github.com/manzaltu/claude-code-ide.el
...
** The =detvdl-emacs-search.el= module
:PROPERTIES:
...
- =M-s o= (~occur~) :: Search for the given regular expression
  ...
  extensions: [[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =detvdl-emacs-search.el= extras provided by the =prot-search.el= library]].
...
*** The =detvdl-emacs-search.el= on isearch lax space
:PROPERTIES:
...
([[#h:e0f9c30e-3a98-4479-b709-7008277749e4][The =detvdl-emacs-search.el= module]]).
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-search.el" :mkdirp yes
...
*** The =detvdl-emacs-search.el= settings for isearch highlighting
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-search.el"
(use-package isearch
  :ensure nil
  :demand t
  :config
...
*** The =detvdl-emacs-search.el= on isearch match counter
:PROPERTIES:
...
*** The =detvdl-emacs-search.el= tweaks to the isearch motion behaviour
:PROPERTIES:
...
[ See: [[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =detvdl-emacs-search.el= extras provided by the =prot-search.el= library]]. ]
...
As of Emacs 28.1, we have the ~isearch-allow-motion~ that activates the
intuitive =M-<=, =M->=, =C-v= and =M-v= movements during an isearch
session.

Alongside ~isearch-allow-motion~, the option
~isearch-motion-change-direction~ was also introduced to work in
conjunction with it.
Setting this to a non-nil value inverts the change direction after any
of the aforementioned movements is executed.
e.g. after =M-<=, the search direction is then forwards 
...
*** The =detvdl-emacs-search.el= tweaks for the occur buffer
:PROPERTIES:
:CUSTOM_ID: h:85aca4da-b89b-4fbe-89e9-3ec536ad7b0d
:END:

Here I am making some minor tweaks to =*occur*= buffer (remember to
read the introduction to this section ([[#h:e0f9c30e-3a98-4479-b709-7008277749e4][The =detvdl-emacs-search.el= module]])).
...
early at startup ([[#h:713ede33-3802-40c6-a8e3-7e1fc0d0a924][The =detvdl-emacs-essentials.el= block that loads my custom libraries]]).
...
*** The =detvdl-emacs-search.el= modified isearch and occur key bindings
:PROPERTIES:
...
*** The =detvdl-emacs-search.el= extras provided by the =prot-search.el= library
:PROPERTIES:
...
- Place the cursor on the opposite end of an isearch when exiting. Do
  it with =C-RET= while in isearch ([[#h:e6fd4ff0-86c7-423b-9cf3-0a9061505136][The =detvdl-emacs-search.el= tweaks to the isearch motion behaviour]]).
...
where windows/buffers are displayed ([[#h:50f8b1e4-b14e-453f-a37e-1c0e495ab80f][The =detvdl-emacs-window.el= rules for displaying buffers (~display-buffer-alist~)]]):
...
- Type =M-s s= (~prot-search-outline~) to use minibuffer completion
  ...
  minibuffer user interface ([[#h:15edf2c3-4419-4101-928a-6e224958a741][The =detvdl-emacs-completion.el= module]]).
  ...
  Consult command on =M-s M-s= ([[#h:22e97b4c-d88d-4deb-9ab3-f80631f9ff1d][The =detvdl-emacs-completion.el= settings for ~consult~]]).
...
- Type =C-u M-s M-t= or =M-s M-T= (~prot-search-git-grep-todo-keywords~)
  ...
  ([[#h:9a3581df-ab18-4266-815e-2edd7f7e4852][The =detvdl-emacs-search.el= setup for editable grep buffers (~wgrep~)]]).
...
matching ([[#h:22e97b4c-d88d-4deb-9ab3-f80631f9ff1d][The =detvdl-emacs-completion.el= settings for ~consult~]]).
...
*** The =detvdl-emacs-search.el= tweaks to ~xref~, ~re-builder~ and ~(dead)grep~
:PROPERTIES:
...
*** The =detvdl-emacs-search.el= setup for editable grep buffers (~grep-edit-mode~ or ~wgrep~)
:PROPERTIES:
...
([[#h:1b53bc10-8b1b-4f68-bbec-165909761e43][The =detvdl-emacs-dired.el= section about ~wdired~ (writable Dired)]]).
...
*** The =detvdl-emacs-search.el= setup for ~phi-search~
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-search.el"
...
*** The =detvdl-emacs-search.el= call to ~provide~
:PROPERTIES:
:CUSTOM_ID: h:c8b2f021-fe5a-4f6b-944c-20340f764fb2
:END:
...
** The =detvdl-emacs-dired.el= module
:PROPERTIES:
...
*** The =detvdl-emacs-dired.el= settings for common operations
:PROPERTIES:
...
([[#h:2e005bd1-d098-426d-91f9-2a31a6e55caa][The =detvdl-emacs-dired.el= section about =trashed.el=]]).

#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-dired.el" :mkdirp yes
...
*** The =detvdl-emacs-dired.el= switches for ~ls~ (how files are listed)
:PROPERTIES:
:CUSTOM_ID: h:679e4460-b306-450f-aa20-497243057e02
:END:

As I already explained, Dired is a layer of interactivity on top of
standard Unix tools ([[#h:f8b08a77-f3a8-42fa-b1a9-f940348889c3][The =detvdl-emacs-dired.el= module]]). We can see
...
- =-h= :: Make file sizes easier to read, such as =555k= instead of
  =568024= (the size of =prot-.org= as of this writing).
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-dired.el"
(use-package dired
  :ensure nil
  :commands (dired)
  :config
...
*** The =detvdl-emacs-dired.el= setting for dual-pane Dired
:PROPERTIES:
...
history ([[#h:25765797-27a5-431e-8aa4-cc890a6a913a][The =detvdl-emacs-completion.el= settings for saving the history (~savehist-mode~)]]).
...
*** The =detvdl-emacs-dired.el= settings to open files externally
:PROPERTIES:
...
*** The =detvdl-emacs-dired.el= miscellaneous tweaks
:PROPERTIES:
...
([[#h:679e4460-b306-450f-aa20-497243057e02][The =detvdl-emacs-dired.el= switches for ~ls~ (how files are listed)]]).
...
([[#h:fbe6f9da-25ee-46a3-bb03-8fa7c1d48dab][The =detvdl-emacs-essentials.el= settings for ~repeat-mode~]]).
...
*** The =detvdl-emacs-dired.el= section about various conveniences
:PROPERTIES:
...
- the key binding for ~dired-do-open~, which opens the file or
  directory externally ([[#h:d7d5e619-3d50-4d9e-a951-7262462a60c9][The =detvdl-emacs-dired.el= settings to open files externally]]).
...
*** The =detvdl-emacs-dired.el= section about my extras (=prot-dired.el=)
:PROPERTIES:
...
given directory ([[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =detvdl-emacs-search.el= extras provided by the =prot-search.el= library]]).
...
*** The =detvdl-emacs-dired.el= section about ~dired-subtree~
:PROPERTIES:
...
*** The =detvdl-emacs-dired.el= section about ~wdired~ (writable Dired)
:PROPERTIES:
:CUSTOM_ID: h:1b53bc10-8b1b-4f68-bbec-165909761e43
:END:

As noted in the introduction, Dired can be made writable
([[#h:f8b08a77-f3a8-42fa-b1a9-f940348889c3][The =detvdl-emacs-dired.el= module]]). This way, we can quickly rename
...
*** The =detvdl-emacs-dired.el= section about ~image-dired~
:PROPERTIES:
...
*** The =detvdl-emacs-dired.el= section about ~dired-preview~
:PROPERTIES:
...
*** The =detvdl-emacs-dired.el= section about multimedia previews (~ready-player~)
:PROPERTIES:
...
packages ([[#h:c345ff72-d905-482a-a885-fc111252aacf][The =detvdl-emacs-dired.el= section about ~dired-preview~]]).
...
*** The =detvdl-emacs-dired.el= section about =trashed.el=
:PROPERTIES:
...
*** The =detvdl-emacs-dired.el= call to ~provide~
:PROPERTIES:
:CUSTOM_ID: h:c8b2f021-fe5a-4f6b-944c-20340f764fb2
:END:
...
** The =detvdl-emacs-window.el= module
:PROPERTIES:
...
*** The =detvdl-emacs-window.el= section about running commands in popup frames
:PROPERTIES:
...
Emacs daemon process or ~server-mode~ ([[#h:7709b7e9-844f-49f3-badf-784aacec4bca][The =detvdl-emacs-essentials.el= arrangement to run Emacs as a server]]).
...
- [[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =detvdl-emacs-org.el= Org capture templates (~org-capture~)]]
- [[#h:6dce278e-86c7-4620-9271-d5af5499f0cc][The =detvdl-emacs-essentials.el= section about ~tmr~ (set timers)]]
- [[#h:7dcbcadf-8af6-487d-b864-e4ce56d69530][The =detvdl-emacs-git.el= section about =project.el=]]
...
*** The =detvdl-emacs-window.el= section about the ~tab-bar-mode~
:PROPERTIES:
...
laptop, whose screen real estate is limited ([[#h:080aa291-95b4-4d54-8783-d156b13190e9][The =detvdl-emacs-essentials.el= section about the laptop-specific settings]]).
...
custom code I have ([[#h:7dcbcadf-8af6-487d-b864-e4ce56d69530][The =detvdl-emacs-git.el= section about =project.el=]]).
...
list of buffers ([[#h:77e4f174-0c86-460d-8a54-47545f922ae9][The =detvdl-emacs-window.el= section about ~beframe~]]).
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-window.el" :mkdirp "yes"
(use-package tab-bar
  :ensure nil
  :config
...
*** The =detvdl-emacs-window.el= section about uniquifying buffer names
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-window.el"
...
*** The =detvdl-emacs-window.el= section about line highlighting (~hl-line-mode~)
:PROPERTIES:
...
*** The =detvdl-emacs-window.el= section about negative space highlighting (~whitespace-mode~)
:PROPERTIES:
:CUSTOM_ID: h:ad2945c5-992d-4c82-a8b8-652f47ebdbea
:END:

Much like ~hl-line-mode~, I normally do not use the ~whitespace-mode~
([[#h:ec156866-3d25-42e7-8247-b190103b6f17][The =detvdl-emacs-window.el= section about line highlighting (~hl-line-mode~)]]).
...
*** The =detvdl-emacs-window.el= section about line numbers (~display-line-numbers-mode~)
:PROPERTIES:
...
*** The =detvdl-emacs-window.el= rules for displaying buffers (~display-buffer-alist~)
:PROPERTIES:
...
*** The =detvdl-emacs-window.el= setting to enable ~visual-line-mode~ in some contexts
:PROPERTIES:
...
*** The =detvdl-emacs-window.el= settings to truncate some buffers silently
:PROPERTIES:
:CUSTOM_ID: h:b6754236-3d08-4448-9c86-3e5923737e4b
:END:

This here is the opposite of what we saw above ([[#h:bf3f50d5-c5db-4704-9498-9e8bfe1b45a8][The =detvdl-emacs-window.el= setting to enable ~visual-line-mode~ in some contexts]]).
...
*** The =detvdl-emacs-window.el= section key bindings
:PROPERTIES:
...
*** The =detvdl-emacs-window.el= configuration of ~undelete-frame-mode~ and ~winner-mode~
:PROPERTIES:
:CUSTOM_ID: h:2df15080-77f9-45f8-a3b5-1adddc70a512
:END:

Since I am using my ~beframe~ package to isolate buffers per frame
([[#h:77e4f174-0c86-460d-8a54-47545f922ae9][The =detvdl-emacs-window.el= section about ~beframe~]]), I appreciate the
...
*** The =detvdl-emacs-window.el= keys for window motions (~ace-window~)
:PROPERTIES:
...
([[#h:fbe6f9da-25ee-46a3-bb03-8fa7c1d48dab][The =detvdl-emacs-essentials.el= settings for ~repeat-mode~]]).
Nevertheless, the built-in ~ace-window~ package provides a set of useful
...
*** COMMENT The =detvdl-emacs-window.el= use of contextual header line (~breadcrumb~)
:PROPERTIES:
...
*** The =detvdl-emacs-window.el= ~provide~ form
:PROPERTIES:
...
** The =detvdl-emacs-git.el= module
:PROPERTIES:
...
*** The =detvdl-emacs-git.el= section about ediff
:PROPERTIES:
...
integrates with ~magit~ ([[#h:b08af527-9ebf-4425-ac3a-24b4f371a4fd][The =detvdl-emacs-git.el= section about ~magit~ (great Git client)]]).
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-git.el" :mkdirp yes
;;;; `ediff'
(use-package ediff
  :ensure nil
...
*** The =detvdl-emacs-git.el= section about =project.el=
:PROPERTIES:
...
built-in =vc.el= ([[#h:50add1d8-f0f4-49be-9e57-ab280a4aa300][The =detvdl-emacs-git.el= section about =vc.el= and related]]).
...
limited to that frame ([[#h:77e4f174-0c86-460d-8a54-47545f922ae9][The =detvdl-emacs-window.el= section about ~beframe~]]).
This does not happen when I am on the laptop though. There I prefer a
singular frame where each projects occupies its own ~tab-bar-mode~ tab ([[#h:b4e65201-3f12-4b24-acb3-a5db48cadb3f][The =detvdl-emacs-window.el= section about the ~tab-bar-mode~]]).
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-git.el"
;;;; `project'
(use-package project
  :ensure nil
  :bind
...
*** The =detvdl-emacs-git.el= section about ~diff-mode~
:PROPERTIES:
...
example, fold the invidual diff hunks and move between them ([[#h:ffff5f7b-a62b-4d4a-ae29-af75402e5c35][The =detvdl-emacs-langs.el= settings for ~outline-minor-mode~]]).
...
completion ([[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =detvdl-emacs-search.el= extras provided by the =prot-search.el= library]]).
...
*** The =detvdl-emacs-git.el= section about =vc.el= and related
:PROPERTIES:
...
([[#h:8b426a69-e3cd-42ac-8788-f41f6629f879][The =detvdl-emacs-git.el= section about ~diff-mode~]]).
...
([[#h:b08af527-9ebf-4425-ac3a-24b4f371a4fd][The =detvdl-emacs-git.el= section about ~magit~ (great Git client)]]).
...
([[#h:257b9118-4407-410e-a56a-134fd5d1a3b4][The =detvdl-emacs-git.el= section about ~agitate~]]).
...
(let ((default-directory (file-truename default-directory)))(buffer-file-name))
*** The =detvdl-emacs-git.el= section about ~magit~ (great Git interface)
:PROPERTIES:
...
all we need to select targets with ease ([[#h:15edf2c3-4419-4101-928a-6e224958a741][The =detvdl-emacs-completion.el= module]]).
...
([[#h:50add1d8-f0f4-49be-9e57-ab280a4aa300][The =detvdl-emacs-git.el= section about =vc.el= and related]]).
...
*** The =detvdl-emacs-git.el= setup for ~diff-hl~
...
*** The =detvdl-emacs-git.el= setup for ~transient~ navigation

For easier sequential navigation of =hunks= in the current file, a
~transient~ prefix is preferable over having to input the =C-x v=
prefix everytime we want to invoke e.g. ~diff-hl-previous-hunk~.

Keeping these actions transient also allows to immediately act on a
hunk when it's navigated to, such as ~diff-hl-stage-current-hunk~.
...
*** The =detvdl-emacs-git.el= call to ~provide~
:PROPERTIES:
...
** The =detvdl-emacs-org.el= module
:PROPERTIES:
...
- Link to files regardless of file type. This includes special links
  ...
  locally and integrated with Emacs ([[#h:755e195b-9471-48c7-963b-33055969b4e2][The =detvdl-emacs-email.el= module]]).
...
*** The =detvdl-emacs-org.el= section on the ~calendar~
:PROPERTIES:
...
([[#h:5812a834-f294-4714-85a5-f8466ececaa8][The =detvdl-emacs-langs.el= extension for Denote Journal extras (~denote-journal~)]]).

Note that Emacs also has a ~diary~ command. I used it for a while, but
Org is far more capable, so I switched to it completely.

#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-org.el" :mkdirp yes
;;; Calendar
...
*** The =detvdl-emacs-org.el= section with basic Org settings
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-org.el"
...
*** The =detvdl-emacs-org.el= section for ~electric~ behaviour
...
*** The =detvdl-emacs-org.el= Org indent mode settings
:PROPERTIES:
...
*** The =detvdl-emacs-org.el= Org to-do and refile settings
:PROPERTIES:
...
which I consider helpful ([[#h:0884658e-9eb5-47e3-9338-66e09004a1a0][The =detvdl-emacs-org.el= Org time/state logging]]).
...
date ([[#h:7fe87b83-2815-4617-a5f9-d3417dd9d248][The =detvdl-emacs-org.el= Org agenda settings]]).
...
the capture phase ([[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =detvdl-emacs-org.el= Org capture templates (~org-capture~)]]).
...
*** The =detvdl-emacs-org.el= Org heading tags
:PROPERTIES:
...
- [[#h:024dd541-0061-4a10-b10b-b17dcd4794b9][The =detvdl-emacs-org.el= Org to-do and refile settings]]
- [[#h:7fe87b83-2815-4617-a5f9-d3417dd9d248][The =detvdl-emacs-org.el= Org agenda settings]]
...
*** The =detvdl-emacs-org.el= Org time/state logging
:PROPERTIES:
...
appointment ([[#h:7fe87b83-2815-4617-a5f9-d3417dd9d248][The =detvdl-emacs-org.el= Org agenda settings]]). This data
...
*** The =detvdl-emacs-org.el= Org link settings
:PROPERTIES:
...
- [[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =detvdl-emacs-org.el= Org capture templates (~org-capture~)]]
- [[#h:49890997-448e-408d-bebe-2003259bb125][The =detvdl-emacs-notmuch.el= glue code for ~org-capture~ (=ol-notmuch.el=)]]
...
[[#h:e03df1e0-b43e-49b5-978e-6a511165617c][The =detvdl-emacs-org.el= section with basic Org settings]].
...
*** The =detvdl-emacs-org.el= Org code block settings
:PROPERTIES:
...
- =#+BEGIN_EXPORT= :: Evaluate the code for the given export target
  ...
  or keeping both of them ([[#h:bd11d4d8-6e9f-4536-87a4-4018783bf8f5][The =detvdl-emacs-org.el= Org export settings]]).
...
*** The =detvdl-emacs-org.el= Org export settings
:PROPERTIES:
...
I showed for Org Babel ([[#h:1f5a0d46-5202-48dd-8048-b48ce17f3df8][The =detvdl-emacs-org.el= Org code block settings]]).
...
*** The =detvdl-emacs-org.el= Org capture templates (~org-capture~)
:PROPERTIES:
...
([[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =detvdl-emacs-search.el= extras provided by the =prot-search.el= library]]).
...
- When I want to capture data that I am not yet sure about, I add it
  ...
  heading in the same file using the ~org-refile~ command ([[#h:024dd541-0061-4a10-b10b-b17dcd4794b9][The =detvdl-emacs-org.el= Org to-do and refile settings]]).
...
- Tasks that have an inherent time component are given a =SCHEDULED=
  ...
  tasks I want to see on my daily agenda ([[#h:7fe87b83-2815-4617-a5f9-d3417dd9d248][The =detvdl-emacs-org.el= Org agenda settings]]).
...
*** The =detvdl-emacs-org.el= Org agenda settings
:PROPERTIES:
...
task in the usual place ([[#h:f8f06938-0dfe-45c3-b4cf-996d36cba82d][The =detvdl-emacs-org.el= Org capture templates (~org-capture~)]]).
...
*** The =detvdl-emacs-org.el= Org Roam settings
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-org.el"
...
*** The =detvdl-emacs-org.el= Org-board settings
...
Source: https://github.com/charlesroelli/org-board
...
*** The =detvdl-emacs-org.el= Org-modern UI settings

This package implements a modern style for your Org buffers using font
locking and text properties. 
The package styles headlines, keywords, tables and source blocks. 
The styling is configurable, you can disable or modify the style of 
each syntax element individually via the org-modern customization group.
...
*** The =detvdl-emacs-org.el= call to ~provide~
:PROPERTIES:
...
** The =detvdl-emacs-langs.el= module
:PROPERTIES:
...
*** The =detvdl-emacs-langs.el= settings for TAB
:PROPERTIES:
...
completion at point ([[#h:804b858f-7913-47ef-aaf4-8eef5b59ecb4][The =detvdl-emacs-completion.el= for in-buffer completion popup (~corfu~)]]).
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-langs.el" :mkdirp yes
...
*** The =detvdl-emacs-langs.el= settings for "electric" behaviour
:PROPERTIES:
...
of characters, I use my ~prot-pair-insert~ command ([[#h:48d2bab5-241f-49dd-9554-8a4dcedce75e][The =detvdl-emacs-essentials.el= section about =prot-pair.el= (insert character pairs)]]).

#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-langs.el"
...
#+end_src
*** The =detvdl-emacs-langs.el= settings for symbol highlighting
=symbol-overlay= provides a visualization of all instances of the symbol under point
...
=highlight-indent-guides= provides a visual indentation indicator for
indent-heavy programming languages such as Python
NOTE: currently HIG is no longer used, due to visual bugs with the
'character fill mode e.g. https://github.com/DarthFennec/highlight-indent-guides/issues/29
- The 'column fill mode works but is not the prettiest
- The 'bitmap fill mode also works, but has problems with the dark
  modus themes and background interpolation
...
#+end_src
*** The =detvdl-emacs-langs.el= settings for ~treesitter~ grammars

Treesitter does not install grammar packages by default.
If possible, we should download the ones we want to use, and later in
the configuration, enable the relevant *-ts modes for these languages
...
*** The =detvdl-emacs-langs.el= settings ~show-paren-mode~
:PROPERTIES:
...
*** The =detvdl-emacs-langs.el= settings for plain text (no double spaces, ~auto-fill-mode~)
:PROPERTIES:
...
command ([[#h:f15bc843-2dd1-4792-95ec-9b7b6e561804][The =detvdl-emacs-essentials.el= section about =prot-simple.el= (custom basic commands)]]).
...
*** The =detvdl-emacs-langs.el= settings for common file types
:PROPERTIES:
:CUSTOM_ID: h:1a132701-9487-4af0-8759-a8d64d02bf1f
:END:

As I explained above about ~auto-mode-alist~ ([[#h:c5fbd6dd-2982-4fca-aeed-d0e750d399bd][The =detvdl-emacs-langs.el= settings for plain text]]),
...
*** The =detvdl-emacs-langs.el= settings for ~eldoc~
:PROPERTIES:
...
*** The =detvdl-emacs-langs.el= settings for ~LSP-mode~ (LSP client)
:PROPERTIES:
:CUSTOM_ID: h:92258aa8-0d8c-4c12-91b4-5f44420435ce
:END:

A typical language server provides the
following capabilities:

- Code completion :: This can be visualised for in-buffer
  automatic expansion of function calls, variables, and the like
  ([[#h:804b858f-7913-47ef-aaf4-8eef5b59ecb4][The =detvdl-emacs-completion.el= for in-buffer completion popup (~corfu~)]]).
...
- Code linting :: To display suggestions, warnings, or errors. These
  ...
  ([[#h:df6d1b52-0306-4ace-9099-17dded11fbed][The =detvdl-emacs-langs.el= settings for code linting (~flymake~)]]).
...
- Code navigation and cross-referencing :: While over a symbol, use a
  ...
  [ Features such as the definition of the outline should, in
    ...
    folding capabilties for outline headings ([[#h:ffff5f7b-a62b-4d4a-ae29-af75402e5c35][The =detvdl-emacs-langs.el= settings for ~outline-minor-mode~]]).
    I usually navigate the outline using minibuffer completion, with
    the help of my ~prot-search-outline~ command ([[#h:b902e6a3-cdd2-420f-bc99-3d973c37cd20][The =detvdl-emacs-search.el= extras provided by the =prot-search.el= library]]). ]
...
Source: https://github.com/minad/cape
...
# TODO: test out https://github.com/blahgeek/emacs-lsp-booster
...
*** The =detvdl-emacs-langs.el= settings for very long lines
:PROPERTIES:
...
*** The =detvdl-emacs-langs.el= settings for ~markdown-mode~
:PROPERTIES:
...
Additionally, we may use edit-indirect to edit Markdown code-blocks in
the same way as we would edit org-src blocks.
...
*** The =detvdl-emacs-langs.el= settings for ~yaml-mode~

As of recent versions of Emacs, Treesitter-specific text modes are
available as natively compiled packages.
...
*** The =detvdl-emacs-langs.el= settings for ~json-ts-mode~
...
*** The =detvdl-emacs-langs.el= settings for ~python~
Python Executable tracking
Source: https://github.com/wyuenho/emacs-pet
...
Flymake-ruff for linting
Source: https://github.com/erickgnavar/flymake-ruff
...
*** The =detvdl-emacs-langs.el= settings for ~ocaml~

Package configuration for the OCaml programming language environment.
See [[https://ocaml.org][OCaml]] for more information on the language and its documentation.

All of the packages mentioned below can be installed using the Opam
package manager for OCaml: [[https://github.com/ocaml/opam][Opam on GitHub]]

OCaml and Opam can be installed and managed through Mise, using the ~asdf:mise~
plugin:
#+BEGIN_EXAMPLE
mise use -g asdf:ocaml @latest
mise use -g asdf:opam@latest
#+END_EXAMPLE

Once installed we do need to initialize it first:
#+begin_example
opam init
#+end_example

With our current ~exec-path-from-shell~ setup, we'll need to move the
opam loading code from our ~.<shell>rc~ to the matching ~.profile~
file, since we load path variables from the login shell and not the
interactive one.
Make sure to add it after the ~mise activate~ call, since opam is only
loaded in scope of the active =mise= session:
...
Normally we'd also need to run ~opam user-setup~, but we are managing
this manually to maintain portability across installations.
The most important part of this command is the addition of an
=.ocamlinit= file with the following contents:
...
The most-used common libraries can also be added to the =.ocamlinit=
file so that they can be used in any toplevel (such as =utop=):
...
The =tuareg= package provides an OCaml major mode that can run the
default toplevel (repl) and debugger within Emacs.
See [[https://github.com/ocaml/tuareg][Tuareg on GitHub]]

The =dune= package provides an interface to the Dune build system for
OCaml.
See [[https://github.com/ocaml/dune][Dune on GitHub]]

The =merlin= package provides an IDE-like experience for OCaml from
within Emacs.
It also integrates well with LSP when the =ocaml-lsp-server= package
is installed.

The =merlin-eldoc= package provides eldoc support for OCaml, deferring
to Merlin internally.

The =flycheck-ocaml= package provides a flycheck-compliant linter for
OCaml, deferring to Merlin internally.

The =utop= package provides improved toplevel (repl) integration from
within Emacs. The primary upgrade from the regular toplevel is that it
provides code completion for functions and variables defined in the OCaml
Emacs buffer.

The =ocamlformat= package provides a standard formatter with some
custom configuration and hooks for improved Emacs support.

All of the required system packages should be installed using ~opam~, as
with the following command:

#+begin_example
opam install tuareg dune utop ocaml-lsp-server merlin ocamlformat
#+end_example
...
ARCHIVED: previous setup with Merlin
#+begin_src emacs-lisp :tangle no
;; (use-package merlin
;;   :ensure t
;;   :after tuareg
...
*** The =detvdl-emacs-langs.el= settings for ~csv-mode~
:PROPERTIES:
...
*** The =detvdl-emacs-langs.el= settings for spell checking
:PROPERTIES:
...
*** The =detvdl-emacs-langs.el= settings for code linting (~flymake~)
:PROPERTIES:
...
LSP linter output ([[#h:92258aa8-0d8c-4c12-91b4-5f44420435ce][The =detvdl-emacs-langs.el= settings for ~eglot~]]).
...
*** The =detvdl-emacs-langs.el= settings for ~outline-minor-mode~
:PROPERTIES:
...
*** The =detvdl-emacs-langs.el= settings for ~dictionary~
:PROPERTIES:
...
*** The =detvdl-emacs-langs.el= settings for ~logos~ (writing extras and buffer navigation)
:PROPERTIES:
...
** The =detvdl-emacs-which-key.el= module
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-which-key.el" :mkdirp yes
...
** The =detvdl-emacs-icons.el= module (~nerd-icons~ for various packages)
:PROPERTIES:
...
#+begin_src emacs-lisp :tangle "detvdl-emacs-modules/detvdl-emacs-icons.el" :mkdirp yes
;;; Icons
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-abbrev.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-comment.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-common.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-dired.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-ediff.el" :mkdirp yes
...
** The =detvdl-electric.el= library
#+begin_src emacs-lisp :tangle "detvdl-lisp/detvdl-electric.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-modeline.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-orderless.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-pair.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-prefix.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-project.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-scratch.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-search.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-shell.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-simple.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-spell.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-vertico.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "detvdl-lisp/prot-window.el" :mkdirp yes
...
** The =detvdl-opam.el= library
#+begin_src emacs-lisp :tangle "detvdl-lisp/detvdl-opam.el"
